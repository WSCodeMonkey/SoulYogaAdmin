...
public class EmployeeAccountServiceImpl implements IEmployeeAccountService {
	...
	public String employeeAccountForgetPassword(String userName) {
		...
		if (UtilValidate.isNotEmpty(newPassword)) {
			...
			try {
				...
				SSLConnectionSocketFactory sslsf = UtilSSL.generateSSL(localCert, keystoreFile, certPassword,
						keyStorePassword);

				// String responceString= new
				// HttpRequestor().jsonPost(UtilProperties.getUtilproperties().readValue("mpi.URL"),
				// parameterMap);
				String responceString = HttpXmlClient.post(UtilProperties.getUtilproperties().readValue("mpi.URL"),
						parameterMap.toString(), sslsf);
				logger.debug("OTP sms message response:------" + responceString);
				/*
				 * JSONObject responceJson=
				 * JSONObject.fromObject(responceString);
			...
			// logger.debug("End admin forget password with new password:
			// "+newPassword) ;
			return newPassword;
		}
		return null;
	...
	public boolean sendCode(String phoneNo) {
		logger.info("Entry class EmployeeAccountServiceImpl method sendCode");

		Integer employeeId = employeeAccountDao.findEmployeeAccountPhoneNo(phoneNo);
		if (UtilValidate.isNotEmpty(employeeId)) {
			logger.info("End class EmployeeAccountServiceImpl method sendCode");
			return true;
		}
